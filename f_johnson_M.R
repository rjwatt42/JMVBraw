f_johnson_M <- function(mu,sd,skew,kurt){
  ...
  # Check for scalars:
  if (sum(c(isscalar(mu),isscalar(sd),isscalar(skew),isscalar(kurt)))<4){
    error('All inputs must be scalars!')
  }
  
  # Check for negative SD:
  if (sd<0) { error('Cannot have a negative SD!') }
  # -------------------------------------
  ...
  while ((ifault==3) || (itype==1 && lambda==-1) || iscomplex(gamma)){
  ...
  result<=list(coef=coef,type=type,msg=msg)
  result
}
...
n_GOTO<-function(where) {
  ...
  return(list(gamma=gamma,delta=delta,lambda=xlam,itype=itype))
}
...
sub_mom <- function(g,d){
  ...
  if (w > expa) {
    fault <- 1
    return(list(hmu=hmu,fault=fault))
  }
  ...
  while ((stop_out==0)){
    ...
    while ((stop_inn==0)){
      ...
      if (cont_inn==1) {
        continue
      }# skip to start of INNER WHILE loop
      v <- rrtpi*h
      for (i in 1:6){
        a(i) <- v*a(i)
      }
